...
class FileOpener:
    ...
    def get_packages_of_frames(self,frames, seq_len: int = 4, size= (412,412)):
        frame_index = 0
        packages_ = []
        while True:
            this_frame_seq = None
            for i in range(frame_index , min(len(frames) , frame_index + seq_len )):
                ...
                if size is None:
                    size = self.get_optimal_out_size(tuple(img.shape[:2]))
                out_img = self.preprocess(img, size).unsqueeze(0)
    ...
    def open_image(self,file, size = (412,412)):
        ...
        out_img = self.preprocess(img, size)
        return out_img


    def open_video(self, file: str, fps: int, seq_len: int = 10, size = None):
        ...
        return self.get_packages_of_frames(frames,seq_len=seq_len,size=size)
